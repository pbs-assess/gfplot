...
for (i in seq_along(spp)) {
  ...
  set.seed(1)
  if (nrow(d_male) > 1000) d_male <- d_male[sample(seq_len(nrow(d_male)), 1000), ]
  if (nrow(d_female) > 1000) d_female <- d_female[sample(seq_len(nrow(d_female)), 1000), ]
  ...
  if (fit_vb_male) {
    ...
    lengths_male <- p_male[["linf"]] * (1 - exp(-p_male[["k"]] * (ages - p_male[["t0"]])))
  }
  if (fit_vb_female) {
  ...
  # Weight:
  d_weight <-  filter(dbio, species_common_name == spp[i], !is.na(length), !is.na(weight))
  ...
  set.seed(42)
  d_plot <- d
  if (nrow(d_plot) > 2000 & nrow(d) > 0) d_plot <- d[sample(seq_len(nrow(d)), 2000), ]
