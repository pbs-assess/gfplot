...
plot_catch <- function(dat, xlim = c(1955, 2017)) {
  ...
  g <- dat %>% 
    ggplot(aes(year, value/1000000, fill = gear)) +
    geom_col() +
    ggsidekick::theme_sleek() +
    scale_fill_manual(values = pal)
  ...
  par(mfrow = c(1, 1), oma = c(1.5, 2.5, 2.5, .5), mar = c(0,0,0,0), cex = 0.8,
  ...
  mtext("Landings (1000 tons)", side = 2, line = 1.5, col = "grey40", cex = 0.75)

  legend(1955, max(gd1$y) * 1.07, legend = levels(all_landings$gear),
    col =  pal, pch = 21, pt.bg = pal, bty = "n", cex = 0.7,
    text.col = "grey40", ncol = 1)
}

dir.create("catches", showWarnings = FALSE)

plyr::d_ply(filter(all_landings), "species_common_name", function(xx) {
