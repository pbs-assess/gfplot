...
mesh6 <- inla.mesh.2d(loc=coords, boundary=bnd, max.edge=c(0.02, 0.2), cutoff=0.02, offset = c(0.1, 0.4))
plot(mesh6)
...
## Now fit the same model with glmmfields and make predictions on the grid:

```{r}
library(glmmfields)
m <- glmmfields(y ~ 1, data = SPDEtoy, lon = "s1", lat = "s2",
  chains = 2, iter = 600, nknots = 15, cores = 2, 
  prior_intercept = student_t(7, 10, 5), cluster = "kmeans",
