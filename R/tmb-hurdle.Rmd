...
- make sure I can reproduce the CPUE index standardization results from recent assessments so I can create them for many stocks and regions for the data synopsis report
- test a delta-lognormal model written in TMB to see if it's considerably faster and obtains the same results
- try a version of the delta-lognormal model that includes random intercepts to see if this is technically feasible in terms of computation time and to see whether it generates a similar standardized index 

In terms of vessel selection criteria right now I am using the same criteria used in the recent walleye pollock assessment:
