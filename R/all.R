...
for(i in tolower(names(rev(sort(table(dcpue$SPECIES_COMMON_NAME)))[1:60]))) {
  print(i)
  pdf(paste0("cpue/", gsub("/", "-", gsub(" ", "-", i)), ".pdf"), width = 3, height = 3)
  plot_spatial_cpue(dcpue, i, bin_width = 7, 
    pal_function = colorRampPalette(c((RColorBrewer::brewer.pal(7, "Blues")))))
  dev.off()
}
...
for(i in unique(spp$species_common_name)) {
  pdf(paste0("sparks/", gsub("/", "-", gsub(" ", "-", i)), ".pdf"), width = 4.5, height = 4.75)
  plot_index_sparks(i)
  dev.off()
}
