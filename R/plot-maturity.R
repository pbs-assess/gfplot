...
#
#   ggplot(nd_fe, aes(age_or_length, glmm_fe, colour = sex)) +
#     geom_line(data = nd,
#       aes(age_or_length, glmm, group = paste(sample_id, sex),
#         colour = sex), inherit.aes = FALSE, alpha = 0.05) +
#     geom_vline(data = labs, aes(xintercept = value, colour = sex), lty = 2,
#       show.legend = FALSE) +
#     geom_line(size = 1.25) +
#     scale_colour_manual(values = c("M" = "grey20", "F" = "red")) +
#     xlab("Age (years)") + ylab("Probability mature") +
#     geom_text(data = labs, aes(x = x, y = y,
