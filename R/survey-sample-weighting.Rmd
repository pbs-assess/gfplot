...
```{r}
d <- readRDS("data-cache/all-commercial-bio.rds") %>% filter(species_common_name %in% "pacific ocean perch") %>% filter(!is.na(age))
ca <- readRDS("data-cache/all-catches.rds") %>% filter(species_common_name %in% "pacific ocean perch")


comps_age_comm <- join_comps_commercial(d, ca, age) %>% weight_comps()
```
