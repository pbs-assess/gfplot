...
The magic happens here with these functions. The first two combine the composition data with information on average species density per stratum and the area of the strata OR .... The second does the re-weighting.
...
```{r}
comp <- survey_specimens %>% filter(!is.na(age)) %>% 
  join_comps_survey(survey_tows) %>% 
  weight_comps()
head(comp)
```
...
```{r}
bio <- readRDS("data-cache/all-commercial-bio.rds") %>% 
  filter(species_common_name %in% "pacific ocean perch")
...
bin_size <- 2
length_bins <-  seq(0, 70, bin_size)

# ------------

bio %>% filter(!is.na(age)) %>% 
  join_comps_commercial(catch, value = age) %>% 
  weight_comps()
bio %>% filter(!is.na(length)) %>% 
  mutate(length_bin = length_bins[findInterval(length, length_bins)] + bin_size/2) %>% 
  join_comps_commercial(catch, value = length_bin) %>% 
  weight_comps()

survey_specimens %>% filter(!is.na(age)) %>% 
  join_comps_survey(survey_tows, value = age) %>% 
  weight_comps()
survey_specimens %>% filter(!is.na(length)) %>% 
  mutate(length_bin = length_bins[findInterval(length, length_bins)] + bin_size/2) %>% 
  join_comps_survey(survey_tows, value = length_bin) %>% 
  weight_comps()
