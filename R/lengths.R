...
tidy_lengths_raw <- function(dat, bin_size = 2,
  ...
  dat <- dat %>%
    select(
      species_common_name, .data$species_science_name, .data$sample_id,
  ...
  all <- expand.grid(
    survey = surveys, year = seq(min(dat$year), max(dat$year)),
  ...
  counts <- select(
    surv_year_counts, .data$total, .data$year,
    .data$survey, .data$max_length
  ) %>% unique()

  max_length <- filter(counts, .data$total >= min_specimens) %>%
    dplyr::pull(.data$max_length) %>%
    max()
