...
areas <- areas[grep(grep_pattern, areas$MAJOR_STAT_AREA_DESCRIPTION), ]
# areas <- areas[grep("5[AB]+|3[CD]+", areas$MAJOR_STAT_AREA_DESCRIPTION), ]
...
caught_something <- d_fe %>%
  ...
  filter(total_positive_tows >= 100) %>%
  filter(spp_catch > 0) %>%
