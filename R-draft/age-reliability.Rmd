...
So aging error does have a moderate effect on $t_0$ (the theoretical average age at length 0) for males or both sexes combined (as shown). We can also see that in the predicted model fits above towards the left of the plot.

There isn't a huge effect of integrating a measurement CV of `r round(mean(sigma), 2)` on ages for females only (not shown).

Another option would be to ignore the between individual aging error and input the ranges of aging error on each observation directly in the final growth model. Or maybe there's some other better way?

These values are approximately in line with those from in Alaska published in:
