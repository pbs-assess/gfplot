SELECT TRIP_START_DATE,
  ...
  G.GROUPING_DESC,
  G.AREA_KM2,
  CASE FE.GEAR_CODE WHEN 1 THEN TRSP.USABILITY_CODE
...
WHERE SPECIMEN_SEX_CODE IN (1, 2) AND TRIP_SUB_TYPE_CODE IN (2, 3) AND
  (FE.FE_PARENT_EVENT_ID IS NULL) AND
  (CASE FE.GEAR_CODE WHEN 1 THEN TRSP.USABILITY_CODE
  WHEN 11 THEN TRSP.USABILITY_CODE
  WHEN 4 THEN LLSP.USABILITY_CODE
  WHEN 5 THEN LLSP.USABILITY_CODE ELSE 0 END) IN (0,1,2,6)
-- insert species here
-- insert ssid here
