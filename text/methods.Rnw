...
\caption{Pacific Ocean Perch CPUE for the groundfish bottom trawl sector. Shown
  is the geometric mean within each hexagon sell since 2013.}
\end{figure}
...
The coloured rectangles indicate the subplot regions in the special survey plots.
...
I then remove duplicate specimen IDs in R since these can be duplicated across
...
and datatype available across all surveys or commercial sources. The cell in
...
(\url{https://github.com/seananderson/glmmfields}). Essentially the Gaussian
...
substrate type. I still need to tweak the interpolation algorithm to go from the
